...
const manageRouter = async () => {
    ...
    if(hash === '#home'){
        ...
        document.title = 'ACT24 Data Platform';
        ...
        if(!element) return;
        if(element.classList.contains('navbar-active')) return;
        document.title = 'ACT24 - Data Access';
        assignNavbarActive(element, 2);
        confluenceDiv.innerHTML = dataRequestTemplate();
    }
        ...
        if(!element) return;
        if(element.classList.contains('navbar-active')) return;
        document.title = 'ACT24 - Survey Details';
        assignNavbarActive(element);
    ...
    // }
        ...
        showAnimation();
        assignNavbarActive(dataDictionaryElement, 2);
        document.title = 'ACT24 - Data Dictionary';
